...
static int8_t pushUartRxData(UART_RXTX_CTL *uartctl, uint8_t *buf, uint16_t len)
{
    ...
    if (uartctl->rxend >= uartctl->rxbegin)
    {
        ...
        if (sparetoend > len)
        {
            tmos_memcpy(uartctl->rxbuf + uartctl->rxend, buf, len);
            uartctl->rxend = (uartctl->rxend + len) % uartctl->rxbufsize;
        }
        else
        {
            tmos_memcpy(uartctl->rxbuf + uartctl->rxend, buf, sparetoend);
...
{
	GPIOA_ModeCfg(LDR1_PIN | LDR2_PIN, GPIO_ModeIN_Floating);
}
