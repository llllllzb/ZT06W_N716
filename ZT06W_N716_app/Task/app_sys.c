...
void LogMessage(uint8_t level, char *debug)
{
    ...
    portUartSend(DEBUG_UART, (uint8_t *)timedebug, strlen(timedebug));
    portUartSend(DEBUG_UART, (uint8_t *)debug, strlen(debug));
    portUartSend(DEBUG_UART, (uint8_t *)"\r\n", 2);
}
...
void LogMessageWL(uint8_t level, char *buf, uint16_t len)
{
    ...
    portGetRtcDateTime(&year, &month, &date, &hour, &minute, &second);
    sprintf(timedebug, "[%02d:%02d:%02d] ", hour, minute, second);
    portUartSend(DEBUG_UART, (uint8 *)timedebug, tmos_strlen(timedebug));
    portUartSend(DEBUG_UART, (uint8 *)buf, len);
    portUartSend(DEBUG_UART, (uint8 *)"\r\n", 2);
}
...
void Log(uint8_t level, const char *debug, ...)
{
    ...
    va_end(args);
    LOGDEBUG[255] = 0;
    portUartSend(DEBUG_UART, (uint8 *)LOGDEBUG, tmos_strlen(LOGDEBUG));
}
...
void LogWL(uint8_t level, uint8 *buf, uint16_t len)
{
    ...
    if (sysinfo.logLevel < level)
        return ;
    portUartSend(DEBUG_UART, buf, len);
}
/*------------------------------------------------------*/
//cmd1完全比配cmd2
