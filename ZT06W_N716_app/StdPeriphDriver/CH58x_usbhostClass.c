...
UINT8 InitRootDevice( void ) 
{
    ...
    LogPrintf(DEBUG_ALL, "Reset host port" );
    ...
    if ( i ) {  										// 复位后设备没有连接
        DisableRootHubPort( );
        LogPrintf(DEBUG_ALL, "Disable host port because of disconnect" );
        return( ERR_USB_DISCON );
    }
    SetUsbSpeed( ThisUsbDev.DeviceSpeed );  // 设置当前USB速度
	
    LogPrintf(DEBUG_ALL, "GetDevDescr: " );
    s = CtrlGetDeviceDescr();  		// 获取设备描述符
    if ( s == ERR_SUCCESS )
    {
        for ( i = 0; i < ((PUSB_SETUP_REQ)SetupGetDevDescr)->wLength; i ++ ) 		
       LogPrintf(DEBUG_ALL, "x%02X ", (UINT16)( Com_Buffer[i] ) );
       ...
       if ( s == ERR_SUCCESS )
       {
           ...
           LogPrintf(DEBUG_ALL, "GetCfgDescr: " );
...
/* 分析配置描述符,获取端点数据/各端点地址/各端点大小等,更新变量endp_addr和endp_size等 */				
               ...
               if ( (dv_cls == 0x00) && (if_cls == USB_DEV_CLASS_STORAGE)) {  // 是USB存储类设备,基本上确认是U盘
                  ...
                  if ( s == ERR_SUCCESS )
                  {
                      ...
                      LogPrintf(DEBUG_ALL, "USB-Disk Ready" );
                      SetUsbSpeed( 1 );  // 默认为全速
                      return( ERR_SUCCESS );
                  }
               }
...
//	需保存端点信息以便主程序进行USB传输
                       ...
                       LogPrintf(DEBUG_ALL, "USB-Print Ready" );
                       SetUsbSpeed( 1 );  // 默认为全速    
                       return( ERR_SUCCESS );
                   }
               }
...
//	进一步初始化,例如设备键盘指示灯LED等
                       LogPrintf(DEBUG_ALL, "USB-Keyboard Ready" );
                       SetUsbSpeed( 1 );  // 默认为全速
                       return( ERR_SUCCESS );
                       }
                       else if ( if_cls == 2 ) {
...
//	为了以后查询鼠标状态,应该分析描述符,取得中断端口的地址,长度等信息
                           LogPrintf(DEBUG_ALL, "USB-Mouse Ready" );
                            SetUsbSpeed( 1 );  // 默认为全速
                           return( ERR_SUCCESS );
                       }
                       s = ERR_USB_UNSUPPORT;
                    ...
                    if(s==ERR_SUCCESS){
                        ...
                        if(s == ERR_SUCCESS){
                            ...
                            LogPrintf(DEBUG_ALL, "USB-HUB Ready" );
                        }
                    }
                }
                else {   // 可以进一步分析
...
//	需保存端点信息以便主程序进行USB传输
    ...
    LogPrintf(DEBUG_ALL, "InitRootDev Err = %02X", (UINT16)s );
#ifdef	FOR_ROOT_UDISK_ONLY
